  ...
  if (userJourneyMap.finalSteps[step]) {
    ...
    journeys?.forEach(journey => {
      ...
      steps.forEach((journeyStep, index) => {
        ...
        const journeyStepEntries = WP.getEntriesByName(journeyStepMetricName);
      ...
      const threshold =
        maxOutlierThreshold ?? config.journeyMaxOutlierThreshold ?? 100000;
      if (everyStepIsMeasured && journeyDuration <= threshold) {
        ...
        if (journeyDuration >= 0) {
          ...
          WP.measure(journeyMetricName, {
            start: finalStepEndTime
              ? finalStepEndTime - journeyDuration
              : finalStepEndTime,
